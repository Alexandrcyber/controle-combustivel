...
function saveCaminhao() {
    ...
    if (isEdit) {
        ...
        if (index !== -1) {
            caminhoes[index] = {
                id: parseInt(caminhaoIdInput.value),
...
function saveAbastecimento() {
    ...
    // Validar campos obrigatórios
    if (!dataInput.value || !caminhaoSelect.value || !motoristaInput.value || 
    ...
    if (isEdit) {
        ...
        if (index !== -1) {
            abastecimentos[index] = {
                id: parseInt(abastecimentoIdInput.value),
...
function resetAbastecimentoForm() {
    ...
    document.getElementById('dataAbastecimento').value = new Date().toISOString().split('T')[0]; // Data atual
...
function confirmDelete() {
    ...
    if (type === 'caminhao') {
        ...
        if (abastecimentosAssociados) {
            ...
            const newAbastecimentos = abastecimentos.filter(a => a.caminhaoId != id);
            abastecimentos = newAbastecimentos;
        }
        
        // Remover caminhão
        const newCaminhoes = caminhoes.filter(c => c.id != id);
        caminhoes = newCaminhoes;
        ...
        const newAbastecimentos = abastecimentos.filter(a => a.id != id);
        abastecimentos = newAbastecimentos;
